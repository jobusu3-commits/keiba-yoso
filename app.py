...
for i in range(n):
    ...
    odds = cols[3].number_input("", min_value=1.0, max_value=999.9, value=max(float(d.get("odds", 10.0)), 1.0), step=0.1, key=f"odds_{i}", label_visibility="collapsed")
    jockey = cols[4].text_input("", value=d.get("jockey", ""), key=f"jockey_{i}", label_visibility="collapsed")
    last_place = cols[5].number_input("", min_value=1, max_value=18, value=max(1, min(d.get("last_place", 5), 18)), key=f"place_{i}", label_visibility="collapsed")
    recent3 = d.get("recent3", [5, 5, 5])
    place2 = cols[6].number_input("", min_value=1, max_value=18, value=max(1, min(recent3[1] if len(recent3) > 1 else 5, 18)), key=f"place2_{i}", label_visibility="collapsed")
    place3 = cols[7].number_input("", min_value=1, max_value=18, value=max(1, min(recent3[0] if len(recent3) > 0 else 5, 18)), key=f"place3_{i}", label_visibility="collapsed")
